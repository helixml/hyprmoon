...
#include "../streaming/rtsp/net.hpp"
...
std::string StreamingEngine::buildPipelineDescription() const {
    ...
    // Use software encoding (nvh264enc causes server hanging)
    // x264enc provides reliable encoding for streaming
    ss << "x264enc bitrate=" << config.video.bitrate << " ! ";
...
void WolfMoonlightServer::startSimpleRTSPServer(std::shared_ptr<state::AppState> app_state) {
    Debug::log(LOG, "WolfMoonlightServer: Starting Wolf RTSP server on port {}", state::get_port(state::RTSP_SETUP_PORT));

    // Use Wolf's battle-tested RTSP server implementation
    std::thread([app_state]() {
        logs::log(logs::warning, "[RTSP] Starting Wolf RTSP server on port {}", state::get_port(state::RTSP_SETUP_PORT));

        try {
            // Use Wolf's proven RTSP server implementation
            rtsp::run_server(state::get_port(state::RTSP_SETUP_PORT), app_state->running_sessions);
        } catch (const std::exception& e) {
            logs::log(logs::error, "[RTSP] Wolf RTSP server error: {}", e.what());
        }

        logs::log(logs::warning, "[RTSP] Wolf RTSP server ended");
    }).detach();

    Debug::log(LOG, "WolfMoonlightServer: Wolf RTSP server started on port {}", state::get_port(state::RTSP_SETUP_PORT));
}
